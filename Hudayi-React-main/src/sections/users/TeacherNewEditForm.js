...
export default function TeacherNewForm({ isEdit, currentUser }) {
  ...
  const getShapeSchema = () => {
    const schema = {
      ...
      // user__identity_number: Yup.string().required(translate('students.required')),
      user__phone: Yup.string().required(translate('students.required')),
      user__gender: Yup.string().required(translate('students.required')),
      // user__birth_date: Yup.string().required(translate('students.required')),
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setIsLoading(true);
      const teachers = await postFormData(
        isEdit ? `teachers/${id}` : 'teachers',
        {
          ...data,
          id: isEdit ? id : '',
          ...
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <RHFTextField
                type="number"
                name="children_count"
                label={translate('teachers.children_count')}
              />
            ...
            <Stack direction="row" sx={{ mt: 3, justifyContent: 'flex-end' }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!isEdit ? translate('teachers.add') : translate('saveChanges')}
              </LoadingButton>
