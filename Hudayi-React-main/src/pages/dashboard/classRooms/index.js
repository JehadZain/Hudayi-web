...
export default function ClassRooms({ isEdit }) {
  ...
  const getClassRooms = useCallback(async (link) => {
    const classRooms = await get(link ?? 'school/class-room/approved', null, enqueueSnackbar);
  ...
  const getClassRoomsPending = useCallback(async (link) => {
    const classRoomsPending = await get(link ?? 'school/class-room/pending', null, enqueueSnackbar);
  ...
  useEffect(() => {
    ...
    getClassRooms();
  }, [getClassRoomsPending, getClassRooms, user.role]);
  ...
  const columns = [
    {
      field: 'id',
      flex: 2,
      headerName: translate('classRooms.classRoomId'),
    },
    {
      field: 'name',
      flex: 2,
...

    {
      ...
      renderCell: (paramsCell) => (
        <Stack direction="row">
          <IconButton onClick={() => push(PATH_DASHBOARD.classRooms.view(paramsCell.row.id))}>
            <Iconify icon="ic:twotone-remove-red-eye" />
          </IconButton>
          {/* <IconButton onClick={() => push(PATH_DASHBOARD.classRooms.modifiye(paramsCell.row.id))}>
    ...
    <>
      ...
      <Container maxWidth={themeStretch ? false : 'xl'}>
        ...
        <Grid container spacing={3}>
          <Grid item xs={6} md={6}>
            <BookingWidgetSummary
              ...
              title={translate('classRooms.classRoomsCount')}
          ...
          <Grid item xs={6} md={6}>
            <BookingWidgetSummary
              ...
              title={translate('classRooms.classRoomsCountPending')}
        ...
        <CustomBreadcrumbs
          ...
          //     variant="contained"
          //     onClick={() => {
          //       push(PATH_DASHBOARD.classRooms.modifiye('new'));
          //     }}
          //     startIcon={<Iconify icon="eva:plus-fill" />}
          //   >
          //     {translate('classRooms.add')}
          //   </Button>
          // }
          sx={{ marginTop: '30px' }}
        />
        ...
        {done ? (
          <Card>
            {user.role !== 'teacher' && (
              ...
              >
                <Tab key="tab1" label={translate('classRooms.classRooms')} value="class-room" />
                <Tab
                  key="tab2"
            ...
            <DataGridCustom
              ...
              onRowClick={(params) => {
                push(PATH_DASHBOARD.classRooms.view(params.row.id));
              }}
              pageSize={pageSize}
              onPageChange={async (newPage) => {
                ...
                if (currentTab === 'class-room/pending') {
                  await getClassRoomsPending(
                    `school/class-room/pending?page=${page}&perpage=${pageSize}`
                  );
                } else {
                  await getClassRooms(
                    `school/class-room/approved?page=${page}&perpage=${pageSize}`
                  );
                }
              }}
              onPageSizeChange={async (newSize) => {
                ...
                if (currentTab === 'class-room/pending') {
                  await getClassRoomsPending(
                    `school/class-room/pending?page=${pageNumber}&perpage=${newSize}`
                  );
                } else {
                  await getClassRooms(
                    `school/class-room/approved?page=${pageNumber}&perpage=${newSize}`
                  );
                }
              }}
              page={pageNumber}
